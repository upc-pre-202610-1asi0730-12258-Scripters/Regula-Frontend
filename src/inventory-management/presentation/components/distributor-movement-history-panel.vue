...
const profileOptions = computed(() => {
  const ids = [...new Set(distributorMovements.value.map((r) => String(r.profileId)).filter(Boolean))].sort()
  return [
    { label: t('inventory.history.filters.all'), value: '' },
...
function providerOrOutbound(row) {
  return row.movementType === 'Entry' ? row.providerName : store.getOutboundTypeLabel(row.outboundType)
}
...
const filteredRows = computed(() =>
    distributorMovements.value.filter((row) => {
      ...
      if (applied.buscar.trim()) {
        ...
        const blob = [row.id, row.cylinderType, providerOrOutbound(row), row.profileId].join(' ').toLowerCase()
        if (!blob.includes(q)) {
          return false
        }
      }
...
<template>
  <div class="inv-page">
    <div>
      <h1 class="inv-title">{{ t('inventory.history.distributor.title') }}</h1>
      <p class="inv-subtitle">{{ t('inventory.history.distributor.subtitle') }}</p>
    </div>
    ...
    <Card>
      ...
      <template #content>
        <div class="inv-filters">
          ...
          <div class="inv-field">
            <span class="inv-label">{{ t('inventory.history.filters.responsible') }}</span>
            <Select v-model="form.profileId" :options="profileOptions" option-label="label" option-value="value" />
          </div>
          <div class="inv-field">
            <span class="inv-label">{{ t('inventory.history.filters.search') }}</span>
            <InputText v-model="form.buscar" />
          </div>
          <Button :label="t('inventory.history.search')" icon="pi pi-search" type="button" @click="applySearch" />
          <Button :label="t('inventory.history.clearFilters')" type="button" severity="secondary" outlined @click="clearFilters" />
        </div>
      </template>
    </Card>
    ...
    <Card>
      <template #title>
        <div class="inv-filters">
          <span>{{ t('inventory.history.distributor.movementsTitle') }} ({{ filteredRows.length }})</span>
          <InputText v-model="keyword" :placeholder="t('inventory.history.keywordPh')" />
        </div>
      </template>
      <template #content>
        <div v-if="!distributorMovementsLoaded" class="inv-loading">
        ...
        <DataTable
            ...
            :rows="25"
        >
          <Column :header="t('inventory.history.cols.idShort')" field="id" />
          ...
          <Column :header="t('inventory.history.distributor.colProvider')">
            <template #body="{ data }">{{ providerOrOutbound(data) }}</template>
          </Column>
          <Column :header="t('inventory.history.filters.responsible')" field="profileId" />
        </DataTable>
      </template>
    </Card>
  </div>
